...
/// Thin BSC wrapper around `EvmClient`.
///
/// Adds:
/// - BSC chain defaults (chain_id)
/// - BEP20 contract address registry
...
///     .unwrap()
///     .register_token(PaymentCurrency::USDT_BEP20, contracts::mainnet::USDT_BEP20);
/// ```
...
impl BscClient {
    ...
    fn is_native(currency: PaymentCurrency) -> bool {
        matches!(currency, PaymentCurrency::BNB)
    }
}
...
impl BlockchainClient for BscClient {
    ...
    async fn estimate_withdrawable(
        ...
        if Self::is_native(request.currency) {
            ...
            // Gas fees are paid in BNB, not the token.
            let contract = self.contract_address(request.currency)?;
            let raw = self
                .evm
